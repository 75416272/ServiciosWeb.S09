...
namespace ServiciosWeb.S09.Controllers
{
    ...
    public class CondicionalesController : ControllerBase
    {
        ...
        {
            if (edad < 0)
                return Ok(new { resultado = "Edad inválida" });

            if (edad <= 12)
                return Ok(new { resultado = "Eres un niño" });

            if (edad <= 17)
                return Ok(new { resultado = "Eres un adolescente" });
        ...
        public IActionResult CalcularDescuento(string tipo, double monto)
        {
            ...
            if (tipo == "vip")
                ...
                return Ok(new { resultado = "Tipo no válido" });
        ...
        public IActionResult DiaSemana(int numero)
        {
            ...
            switch (numero)
            {
                ...
                default:
                    return Ok(new { resultado = "Número inválido (1-7)" });
            }
